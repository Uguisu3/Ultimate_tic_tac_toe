...
class TreeNode:
    def __init__(self, heuristicVal, player, spotsTemp, totalSectionsTemp, prevMove = None):
        ...
        self.children = None
        self.player = player
        self.spotsTemp = spotsTemp
        self.totalSectionsTemp = totalSectionsTemp
...
def miniMax(currentNode, depth, maximizingPlayer, posMoves, alpha, beta):
    ...
    for pos in posMoves:
        ...
        spotsTempCopy = copy.deepcopy(currentNode.spotsTemp)
        totalSectionsTempCopy = copy.deepcopy(currentNode.totalSectionsTemp)

        spotsTempCopy[x][y] = currentNode.player
        done = check_win_sect(x, spotsTempCopy, totalSectionsTempCopy, currentNode.player)
    ...
    if(maximizingPlayer):
        ...
        for child in currentNode.children:
            childSubtree = miniMax(child, depth - 1, False, move_list(child.prevMove, child.spotsTemp), alpha, beta)
            maxEval = max(maxEval, childSubtree.minMaxVal)
            alpha = max(alpha, childSubtree.minMaxVal)
            if(beta <= alpha):
                break
        currentNode.minMaxVal = maxEval
        return currentNode
    else:
        minEval = 10000000000
        for child in currentNode.children:
            childSubtree = miniMax(child, depth - 1, True, move_list(child.prevMove, child.spotsTemp), alpha, beta)
            minEval = min(minEval, childSubtree.minMaxVal)
            beta = min(beta, childSubtree.minMaxVal)
            if (beta <= alpha):
                break
        currentNode.minMaxVal = minEval
        return currentNode
...
def check_win_sect(section, spots, total_sections,
                   player, gameComplete = False):  # updates total_sections, and checks if entire game has been won
...
def heuristic_function(currentMove, spots, player, total_sections):
    ...
    if (check_win_sect(xTemp, spotsTemp, totalSectionsTemp, player)):
        ...
        if (heuristic_temp == 300):
            heuristic_val = 10
            heuristic_val += (getHeuristicValNoOptimalMove(
                currentMove) / 10)  # move results in multiple possible wins.  Corner and center moves are
...
def checkTwoInARowBig(total_sections, winningBoard, player):
    ...
    for inLineSpot in inLine:
        if (total_sections[inLineSpot] == player):  # if other inline spot is owned by player
            if (isNotImpeded(winningBoard, inLineSpot,
                             total_sections)):  # if spot aka winningBoard and other owned inline spot are not blocked by the other player
                numberOfInARows += 1
...
def isNotImpeded(spot1, spot2, total_sections):
    ...
    emptyBoard[spot2] = 3
    movesToCheck = [0, 1, 2, 3, 4, 5, 6, 7,
                    8]  # check all the spots not including spot1 and spot2 to see if it would result
...
def getHeuristicValNoOptimalMove(currentMove):
    ...
    col = currentMove[0]
    if (
            row == 0 or row == 2 or row == 3 or row == 5 or row == 6 or row == 8):  # a top or bottom row on individual board
        topOrBottomRow = True
    else:
        topOrBottomRow = False
    if (
            col == 0 or col == 2 or col == 3 or col == 5 or col == 6 or col == 8):  # a top or bottom col on individual board
        topOrBottomCol = True
    else:
...
def main(playertype):
    ...
    while not done:
        ...
        if (currentplayer == 1):
            ...
            miniMaxTree = miniMax(currentNode, 4, True, open_pos, -10000000000, 10000000000)

            if ((miniMaxTree.children != None) and (miniMaxTree.children != [])):
                for posNode in miniMaxTree.children:
                    currentPosValues.append(posNode.minMaxVal)
                currentMoveIndex = currentPosValues.index(miniMaxTree.minMaxVal)
                pos = open_pos[currentMoveIndex]
            ...
            # check if the move caused the player to win a section
            # if the move won the whole game done will become true
            done = check_win_sect(x, spots, total_sections, player, True)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    done = True
                elif event.type == pygame.KEYDOWN:
