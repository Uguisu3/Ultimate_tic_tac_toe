...
def heuristic_function(currentMove, spots, player, total_sections):
    ...
    if (check_win_sect(xTemp, spotsTemp, totalSectionsTemp, player)):
        ...
        if(True): #Leads to 2 in a row in grand scheme.  Need to write this function
            heuristic_val = 50
        else:
            heuristic_val = 25
    else:
        #can maybe comment out if entirely
        if (False): #Leads to 2 in a row in smaller board.  Need to write this function.  Same as function above?
            heuristic_val = 10
